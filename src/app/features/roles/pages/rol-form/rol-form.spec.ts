...
describe('RolFormPage', () => {
    ...
    beforeEach(async () => { store.initialize.mockClear(); await TestBed.configureTestingModule({ imports: [RolFormPage], providers: [provideRouter([]), { provide: ActivatedRoute, useValue: { snapshot: { paramMap: { get: () => null } } } }, { provide: FeedbackService, useValue: feedback }, { provide: AuthStore, useValue: { hasPermission: vi.fn(() => true) } }] }).overrideComponent(RolFormPage, { set: { providers: [{ provide: RolFormStore, useValue: store }] } }).compileComponents(); fixture = TestBed.createComponent(RolFormPage); fixture.detectChanges(); });
