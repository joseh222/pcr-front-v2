...
export class RolFormPage implements OnInit {
    protected readonly store = inject(RolFormStore); private readonly route = inject(ActivatedRoute); private readonly router = inject(Router); private readonly fb = inject(FormBuilder); private readonly feedback = inject(FeedbackService);
    protected readonly idRole = signal<number | null>(null); protected readonly isEditMode = computed(() => this.idRole() !== null); protected readonly detail = computed(() => this.store.detail()); protected readonly isSystem = computed(() => !!this.detail()?.isSystem); protected readonly grantsAllPermissions = computed(() => !!this.detail()?.grantsAllPermissions); protected readonly permissionsLocked = computed(() => this.grantsAllPermissions()); protected readonly pageTitle = computed(() => this.isEditMode() ? 'Rol y permisos' : 'Nuevo rol'); protected readonly pageDescription = computed(() => this.isEditMode() ? 'Consulta el rol y administra los permisos que tiene asignados.' : 'Crea un rol personalizado y define sus permisos de acceso.');
    ...
    private readonly syncDetail = effect(() => {
        ...
        this.form.patchValue({ code: detail.code, name: detail.name, description: detail.description ?? '', permissions: [...this.store.assignedPermissionIds()] }, { emitEvent: false });
        this.form.controls.code.disable({ emitEvent: false });
        if (detail.isSystem) { this.form.controls.name.disable({ emitEvent: false }); this.form.controls.description.disable({ emitEvent: false }); }
        if (detail.grantsAllPermissions) this.form.controls.permissions.disable({ emitEvent: false });
    });
    ...
    ngOnInit(): void {
        ...
        const id = Number(rawId); if (!Number.isInteger(id) || id <= 0) { void this.router.navigate(['/seguridad/roles']); return; }
        this.idRole.set(id); this.store.initialize(id);
    }

    protected save(): void {
        this.form.markAllAsTouched(); if (this.form.invalid || this.store.saving() || this.grantsAllPermissions()) return;
        ...
        if (id === null) { this.store.create({ code: value.code.trim().toUpperCase(), name: value.name.trim(), description }, permissions); return; }
        this.store.update(id, value.name.trim(), description, permissions);
    }
