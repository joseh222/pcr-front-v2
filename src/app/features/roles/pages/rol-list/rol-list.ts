...
export class RolListPage implements OnInit {
    protected readonly store = inject(RolListStore); private readonly fb = inject(FormBuilder); private readonly actions = inject(RolActionsService); private readonly authStore = inject(AuthStore);
    protected readonly canCreate = computed(() => this.authStore.hasPermission(PERMISSION_CODE.ROLE_CREATE)); protected readonly canChangeStatus = computed(() => this.authStore.hasPermission(PERMISSION_CODE.ROLE_STATUS));
    ...
    protected changeStatus(role: RolListItem): void { if (!this.canChangeStatus() || role.isSystem) return; this.actions.changeStatus(role).subscribe(changed => { if (changed) this.store.reload(); }); }
