...
import { BautismoListItem } from '../../../bautismos/data-access/models/bautismo.models';
...
export class LibroSacramentalDetailPage implements OnInit {
    private readonly api = inject(LibroSacramentalApiService); private readonly bautismoApi = inject(BautismoApiService); private readonly actions = inject(LibroSacramentalActionsService); private readonly authStore = inject(AuthStore); private readonly fb = inject(FormBuilder); private readonly id = Number(inject(ActivatedRoute).snapshot.paramMap.get('id') ?? 0);
    protected readonly loading = signal(true); protected readonly foliosLoading = signal(false); protected readonly partidasLoading = signal(false); protected readonly error = signal<string | null>(null); protected readonly folioError = signal<string | null>(null); protected readonly partidasError = signal<string | null>(null); protected readonly book = signal<LibroSacramentalDetail | null>(null); protected readonly folios = signal<readonly FolioSacramental[]>([]); protected readonly selectedFolio = signal<FolioSacramental | null>(null); protected readonly partidas = signal<readonly BautismoListItem[]>([]); protected readonly page = signal(0); protected readonly pageSize = 60;
    protected readonly canEdit = computed(() => this.authStore.hasPermission(PERMISSION_CODE.SACRAMENTAL_BOOK_EDIT) && this.book()?.codigoEstadoDigitalizacion !== 'COMPLETADA'); protected readonly canChangeState = computed(() => this.authStore.hasPermission(PERMISSION_CODE.SACRAMENTAL_BOOK_STATE)); protected readonly canReopen = computed(() => this.authStore.hasPermission(PERMISSION_CODE.SACRAMENTAL_BOOK_REOPEN)); protected readonly canCreateBaptism = computed(() => this.authStore.hasPermission(PERMISSION_CODE.BAPTISM_CREATE) && this.book()?.codigoTipoSacramento === 'BAUTISMO' && this.book()?.codigoEstadoDigitalizacion === 'EN_PROCESO'); protected readonly canViewBaptism = computed(() => this.authStore.hasPermission(PERMISSION_CODE.BAPTISM_VIEW));
    ...
    protected searchFolio(): void { this.page.set(0); this.clearSelectedPartidas(); this.loadFolios(this.folioForm.controls.numeroFolio.value); }
    protected clearFolioSearch(): void { this.folioForm.reset({ numeroFolio: '' }); this.page.set(0); this.clearSelectedPartidas(); this.loadFolios(); }
    protected selectFolio(folio: FolioSacramental): void { this.selectedFolio.set(folio); this.loadPartidas(folio); }
    ...
    private loadFolios(numeroFolio?: string | null): void { this.foliosLoading.set(true); this.folioError.set(null); this.api.getFolios(this.id, numeroFolio).subscribe({ next: folios => { this.folios.set(folios); this.foliosLoading.set(false); if (folios.length === 1) this.selectFolio(folios[0]); else if (this.selectedFolio() && !folios.some(item => item.idFolioSacramental === this.selectedFolio()!.idFolioSacramental)) this.clearSelectedPartidas(); }, error: error => { this.folios.set([]); this.clearSelectedPartidas(); this.folioError.set(getApiErrorMessage(error, 'No se pudieron cargar los folios.')); this.foliosLoading.set(false); } }); }
    private loadPartidas(folio: FolioSacramental): void { this.partidas.set([]); this.partidasError.set(null); if (this.book()?.codigoTipoSacramento !== 'BAUTISMO' || !this.canViewBaptism()) return; this.partidasLoading.set(true); this.bautismoApi.getByFolio(folio.idFolioSacramental).subscribe({ next: x => { this.partidas.set(x); this.partidasLoading.set(false); }, error: e => { this.partidasError.set(getApiErrorMessage(e, 'No se pudieron cargar las partidas del folio.')); this.partidasLoading.set(false); } }); }
    private clearSelectedPartidas(): void { this.selectedFolio.set(null); this.partidas.set([]); this.partidasError.set(null); this.partidasLoading.set(false); }
}
