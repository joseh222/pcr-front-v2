...
describe('BautismoFormPage',()=>{
    ...
    beforeEach(()=>{ TestBed.overrideProvider(Router, { useValue: routerMock });vi.clearAllMocks();api.getSiguientePartida.mockReturnValue(of({idLibroSacramental:7,numeroLibro:'11',ultimaPartidaNumerica:157,siguientePartidaSugerida:158}));libros.getTiposSacramento.mockReturnValue(of([{idTipoSacramento:1,codigo:'BAUTISMO',nombre:'Bautismo',orden:1}]));libros.getList.mockReturnValue(of([book]));libros.getFolios.mockReturnValue(of([folio]));dialog.open.mockReturnValue({afterClosed:()=>of(true)} as any);TestBed.configureTestingModule({imports:[BautismoFormPage],providers:[provideRouter([]), { provide: Router, useValue: routerMock },{provide:ActivatedRoute,useValue:{snapshot:{paramMap:convertToParamMap({}),queryParamMap:convertToParamMap({libro:'7',folio:'23'})}}},{provide:BautismoApiService,useValue:api},{provide:LibroSacramentalApiService,useValue:libros},{provide:FeedbackService,useValue:feedback},{provide:MatDialog,useValue:dialog},{provide:SacramentalTextCaseService,useValue:{forzarMayusculas:()=>true,ensureLoaded:vi.fn()}}]});});
