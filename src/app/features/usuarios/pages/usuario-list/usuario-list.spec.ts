...
describe('UsuarioListPage', () => {
    ...
    beforeEach(async () => {
        await TestBed.configureTestingModule({ imports: [UsuarioListPage], providers: [provideRouter([]), { provide: AuthStore, useValue: { currentUser: signal({ idUser: 1 }), grantsAllPermissions: signal(true), hasPermission: vi.fn(() => true), hasAllPermissions: vi.fn(() => true) } }, { provide: UsuarioActionsService, useValue: actions }] }).overrideComponent(UsuarioListPage, { set: { providers: [{ provide: UsuarioListStore, useValue: store }] } }).compileComponents();
