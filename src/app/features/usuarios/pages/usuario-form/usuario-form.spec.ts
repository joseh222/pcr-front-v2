...
describe('UsuarioFormPage', () => {
    ...
    beforeEach(async () => {
        await TestBed.configureTestingModule({ imports: [UsuarioFormPage], providers: [provideRouter([]), { provide: ActivatedRoute, useValue: { snapshot: { paramMap: { get: () => null } } } }, { provide: PersonaApiService, useValue: { search: vi.fn(() => of([])) } }, { provide: AuthStore, useValue: { currentUser: signal({ idUser: 1 }), hasPermission: vi.fn(() => true) } }, { provide: FeedbackService, useValue: { success: vi.fn(), error: vi.fn(), warning: vi.fn() } }, { provide: MatDialog, useValue: { open: vi.fn() } }] }).overrideComponent(UsuarioFormPage, { set: { providers: [{ provide: UsuarioFormStore, useValue: store }] } }).compileComponents();
