...
export class UsuarioFormPage implements OnInit {
    ...
    private readonly syncRoleAccess = effect(() => { this.isEditMode(); this.isSelf(); this.canAssignRoles(); this.syncRoleControl(); });
    ...
    ngOnInit(): void {
        ...
        this.idUser.set(id); this.syncRoleControl(); this.store.initialize(id);
    }
    ...
    private syncRoleControl(): void { if (this.isSelf() || !this.canAssignRoles()) this.form.controls.roles.disable({ emitEvent: false }); else this.form.controls.roles.enable({ emitEvent: false }); }
