...
describe('ConfiguracionMantenimientosPage',()=>{
    it('carga parroquia, catálogos, series y tarifas',()=>{
        ...
        expect(text).toContain('PARROQUIA TEST');expect(text).toContain('Efectivo');expect(text).toContain('T001');expect(text).toContain('Precios de Misas');expect(text).toContain('S/ 50.00');
    });
});
