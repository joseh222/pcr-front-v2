...
describe('MovimientoListPage', () => {
    ...
    const inventoryApiMock = { getTiposMovimientoHistorial: vi.fn(() => of([])), getProductos: vi.fn(() => of({ items: [], pageNumber: 1, pageSize: 100, totalRecords: 0, totalPages: 0 })) };
    const feedbackMock = { warning: vi.fn() };
    const routerMock = { navigate: vi.fn(() => Promise.resolve(true)) };
    beforeEach(() => { Object.values(storeMock).forEach(v => { if (typeof v === 'function' && 'mockClear' in v) (v as any).mockClear(); }); inventoryApiMock.getTiposMovimientoHistorial.mockClear(); inventoryApiMock.getProductos.mockClear(); feedbackMock.warning.mockClear(); routerMock.navigate.mockClear(); TestBed.configureTestingModule({ imports: [MovimientoListPage], providers: [{ provide: AuthStore, useValue: authStoreMock }, 
        { provide: ActivatedRoute, useValue: { snapshot: { queryParamMap: convertToParamMap({}) } } },
        { provide: InventarioApiService, useValue: inventoryApiMock }, { provide: FeedbackService, useValue: feedbackMock }, { provide: Router, useValue: routerMock }
