...
import { ResumenEconomicoFilters, ResumenEconomicoResponse } from '../../data-access/resumen-economico.models';

@Component({
    selector: 'pcr-resumen-economico',
    ...
    styleUrl: './resumen-economico.scss'
})
export class ResumenEconomicoPage implements OnInit {
    private readonly fb = inject(FormBuilder);
    private readonly api = inject(ResumenEconomicoApiService);
    ...
    protected readonly report = signal<ResumenEconomicoResponse | null>(null);
    protected readonly maxMetodoPago = computed(() => Math.max(0, ...(this.report()?.metodosPago.map(item => item.totalIngresos) ?? [])));
    protected readonly maxComposicion = computed(() => Math.max(0, ...(this.report()?.composicionVentas.map(item => item.totalDetalle) ?? [])));
    protected readonly maxFlujo = computed(() => Math.max(0, ...(this.report()?.flujoDiario.flatMap(item => [item.ingresos, item.egresos]) ?? [])));
    ...
    protected search(): void {
        ...
        if (!filters.fechaInicio || !filters.fechaFin) { this.feedback.warning('Selecciona el rango de fechas del resumen.'); return; }
    ...
    protected currentMonth(): void {
        this.filterForm.reset(this.currentMonthRange());
        this.load();
    }
    ...
    protected exportExcel(): void {
        ...
        this.api.exportExcel(this.currentFilters()).pipe(finalize(() => this.exportingExcel.set(false))).subscribe({
            next: blob => { this.fileDownload.download(blob, this.buildExportFileName('xlsx')); this.feedback.success('Excel del resumen económico generado correctamente.'); },
    ...
    protected exportPdf(): void {
        ...
        this.api.exportPdf(this.currentFilters()).pipe(finalize(() => this.exportingPdf.set(false))).subscribe({
            next: blob => { this.fileDownload.download(blob, this.buildExportFileName('pdf')); this.feedback.success('PDF del resumen económico generado correctamente.'); },
            error: error => this.feedback.error(getApiErrorMessage(error, 'No se pudo exportar el reporte a PDF.'))
        });
    }

    protected canExport(): boolean { return (this.report()?.cantidadVentas ?? 0) > 0 || (this.report()?.cantidadCompras ?? 0) > 0; }
    ...
    private load(): void {
        ...
        this.api.get(filters).pipe(finalize(() => this.loading.set(false))).subscribe({
            ...
            error: error => this.feedback.error(getApiErrorMessage(error, 'No se pudo cargar el resumen económico.'))
        });
    }

    private currentFilters(): ResumenEconomicoFilters {
    ...
    private buildExportFileName(extension: 'xlsx' | 'pdf'): string {
        ...
        return `Resumen_Economico_${stamp}.${extension}`;
    }
