...
export class SolicitudServicioFormStore {
    ...
    initialize(idSolicitudServicio: number | null): void {
        ...
        forkJoin({ tipos: this.api.getPersonaTiposDocumento(), detail: detail$ }).subscribe({
            next: result => {
                this.tiposDocumentoSignal.set(result.tipos);
                this.detailSignal.set(result.detail);
                if (result.detail) {
                    this.api.getServicioById(result.detail.idServicio).subscribe({
    ...
    searchServices(search: string): void {
        ...
        this.api.searchServicios(text, 10).pipe(catchError(() => of([] as readonly ServicioLookupItem[]))).subscribe(items => {
    ...
    searchPersons(search: string): void {
        ...
        this.api.searchPersonas(text, 10).pipe(catchError(() => of([] as readonly PersonaSearchItem[]))).subscribe(items => {
    ...
    findPersonByDocument(idTipoDocumento: number, numeroDocumento: string): void {
        ...
        this.api.getPersonaByDocument(idTipoDocumento, numeroDocumento.trim()).subscribe({
            next: person => {
    ...
    createPerson(request: PersonaCreateRequest): void {
        this.personErrorSignal.set(null);
        this.api.createPersona(request).subscribe({
